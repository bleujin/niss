...
public abstract class FieldIndexingStrategy {
	...
	public static final FieldIndexingStrategy DEFAULT = new FieldIndexingStrategy() {
		...
		public void save(Document doc, MyField myField, final Field ifield) {
			...
			if (myField.myFieldtype() == MyFieldType.Unknown) {
				if (NumberUtil.isNumber(myField.unknownValue())) {
					doc.add(new DoubleField(fieldName, NumberUtil.toDouble(myField.unknownValue()), Store.NO));
				} else if (StringUtil.isAlphanumericUnderbar(myField.unknownValue())) {
					doc.add(new StringField(fieldName, myField.unknownValue(), Store.YES));
				}
			} else {
				if (myField.myFieldtype() == MyFieldType.Number) {
					...
					doc.add(new StringField(fieldName, ifield.stringValue(), Store.NO));
					doc.add(new LongField(fieldName, NumberUtil.toLong(DateUtil.dateToString(date, "yyyyMMdd")), Store.NO));
				}
			}
