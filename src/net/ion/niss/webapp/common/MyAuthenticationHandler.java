...
public class MyAuthenticationHandler extends AbstractHttpHandler {
	...
	private void needAuthentication(HttpResponse response) {
		...
		// response.status(302).header(HttpHeaders.LOCATION, "/login.html").end() ;
		// response.content(IOUtil.toStringWithClose(new FileInputStream("./webapps/admin/login.html"))).end() ;
	}
